...
namespace index_manager {
    auto fetch_data(FILE::path &&__path, const bool __mod = false /* Flag for fetching only modified files*/) -> FILE_PACK {
        FILE_PACK __data;
        ...
        std::ranges::for_each(fetched_data, [&](const std::string &__val) -> void {
            bool filemod = false;
            std::unique_ptr<File> f = std::make_unique<File>(std::move(__val));
            if (f->get_HashID() != static_cast<std::string>(fetched_data[__val]["Hash"]) ||
                ...
                f->get_Size() != static_cast<ssize_t>(fetched_data[__val]["Size"])) {
                filemod = true;
            }
